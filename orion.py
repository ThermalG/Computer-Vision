...
# buffer = OF_Buffer(size = 10)   # depending on available memory. adjust along with S and CONTIGUITY
def optic_flow(frame_prev, frame, gpu=False, roi=.5):  # for GPU, only NVIDIA platforms supported
    ...
    while source.isOpened():  # REF[1][3]
        ...
        if ctr % S == 0:
            ...
            if frame_prev is not None:  # REF[4][5]
                ...
                cv2.putText(frame, f'Speed: {s:.1f} KPH', (int(.05 * size[0]), int(.05 * size[1])), 2, 2 * C,
                            (255, 0, 127), int(2 * C)) if WRITE_VIDEO else None
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for b_id, p_pos in p_stack.items():  # rule out cyclists
                    for b_pos in b_stack.items():  # REF[6]
                        if all(len(x) >= CONTIGUITY for x in [b_pos, p_pos]) and all(
                                abs(p_pos[-i][0] - b_pos[-i][0].item()) + abs(
                                    p_pos[-i][1] - b_pos[-i][1].item()) < PROXIMITY for i in
                                range(1, CONTIGUITY + 1)):
                            cyclist.add(b_id)
                            break
            # update counts
